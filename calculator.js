...
function calculator( button ){
    if( button.type == "operator" ){
        ...
        // write values of array result as string in variable
        let result_joined = data.result.join('');

        // clear all arrays -> no more needed
        data.operation = [];
        data.result = [];

        // write final result in variable
        // eval -> function that calculates everything
        let result_final;

        // check for syntax error, eg: 6+*5
        try {
        ...
        // if result_final is too big for output screen
        result_final = formatResult(result_final);

        // save result if needed later
